...
import { Bus, Clock, ArrowRight, Check, Zap } from 'lucide-react';
import { Container, Section } from '@/components/ui';
import { getPackagesByCategory } from '@/lib/data/packages';
import { formatPrice } from '@/lib/utils';

const statLabels: Record<string, string> = {
  rollerZiplineMetres: 'm Total',
  speed: 'km/h Max',
  drops: 'Drops',
  duration: 'Duration',
};

export default function RollerPackagesPage() {
  const packages = getPackagesByCategory('roller');

  return (
    <main className="min-h-screen">
      {/* Hero Section */}
      <section className="relative py-24 bg-[#1a1a1a] overflow-hidden">
        <div className="absolute inset-0 bg-[url('/images/Hero%20Image/Roller.jpg')] bg-cover bg-center opacity-20" />
        ...
        <Container className="relative z-10">
          ...
          >
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-[family-name:var(--font-trade-winds)] text-white mb-6">
              Roller <span className="text-[#f2e421]">Zipline</span>
            </h1>
            <p className="text-lg text-white/70">
              A unique fusion of roller coaster and zipline. Feel the rush as you twist and turn 
              through the trees on Thailand&apos;s first roller zipline.
            </p>
          </motion.div>
        </Container>
      </section>
